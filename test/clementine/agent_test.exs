defmodule Clementine.AgentTest do
  ...
  describe "stream/2" do
    test "emits real streaming events and updates history" do
      ...
      # Run events are now {seq, event} tuples; {:done, _} is not seq-wrapped.
      assert Enum.any?(events, &match?({_seq, {:text_delta, "Hello "}}, &1))
      assert Enum.any?(events, &match?({_seq, {:text_delta, "stream!"}}, &1))
      assert List.last(events) == {:done, :success}

      # seq is per-run, starts at 0, strictly increasing and contiguous across
      # ALL run events (loop_event wrappers + text deltas), excluding the
      # terminal {:done, _} marker which the stream itself generates.
      seqs =
        events
        |> Enum.flat_map(fn
          {seq, _inner} when is_integer(seq) -> [seq]
          {:done, _} -> []
        end)
      ...
      # Non-text run events carry seq too (loop_event wrappers are present).
      assert Enum.any?(events, &match?({_seq, {:loop_event, _}}, &1))
    ...
    test "emits streaming errors once before done" do
      ...
      # Run events are seq-wrapped (per-run, starting at 0, contiguous); the
      # terminal {:done, :error} marker is generated by the stream and is bare.
      assert events == [
               {0, {:loop_event, {:loop_start, "Hi"}}},
               {1, {:loop_event, {:iteration_start, 1}}},
               {2, {:loop_event, :llm_call_start}},
               {3, {:text_delta, "partial"}},
               {4, {:error, error}},
               {5, {:loop_event, {:llm_call_end, {:error, error}}}},
               {6, {:loop_event, {:loop_end, {:error, error}}}},
               {:done, :error}
             ]
  ...
  describe "run_async/2" do
    ...
    test "rejects a synchronous run while an async run is already running" do
      ...
      assert {:error, {:agent_busy, [^task_id]}} = Clementine.Agent.run(agent, "Sync task")
