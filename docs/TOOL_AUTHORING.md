...
When the rollout acts on the model's tool calls, before any call is gated or executed, `Clementine.ToolInput.repair/3` splits such input back apart, so an approval-gated call is approved with exactly the arguments that will run (and a resumed approval runs the checkpointed arguments, never a re-derivation). History keeps the model's own calls, and `ToolRunner` passes arguments through unchanged. The repair: the garbled parameter keeps the text before the stray tag, and each embedded parameter your tool declares, and the input lacks, gets its value decoded to the declared type. Values the provider delivered as real fields (even `""` or `nil`) are never overwritten, only top-level string parameters are repaired, and only the call syntax's own tags are removed: values are kept byte for byte, including an email's `</p>` or a file's indentation and trailing newline. A value can also end in the call syntax with no parameter after it (`"emergency</tier>\n</submit_assessment>"`, `"...</summary>\n</invoke>"`). Its own closing tag (or a `parameter` tag) is cut off, together with the closers after it, as long as at least one of those closers is call syntax: `</invoke>`, `</function_calls>`, a `parameter` tag, or the tool's own name. A lone closing tag stays, so an HTML email body that ends in `</body>` keeps it. Each repair emits `[:clementine, :tool, :input_repaired]` with the parameters it touched, so you can count them.
