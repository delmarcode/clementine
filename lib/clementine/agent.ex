defmodule Clementine.Agent do
  ...
  monotonically increasing non-negative integer (starting at `0`) attached by
  `Clementine.Loop.run_stream/3`. It covers every forwarded event type
  (text deltas, tool use, tool results, errors, and wrapped loop events), so a
  downstream observer can dedupe and reorder on reconnect.

  The terminal `{:done, :success}` / `{:done, :error}` markers are generated by
  the stream itself (not the loop) and are NOT seq-wrapped. The `{:error,
  reason}` emitted when starting the stream fails (e.g. `{:agent_busy, _}`) is
  likewise produced by the stream resource and carries no seq.
  """
  ...
  defp next_stream_event(
    ...
    receive do
      {:clementine_stream_event, ^task_id, {_seq, {:error, _reason}} = event} ->
