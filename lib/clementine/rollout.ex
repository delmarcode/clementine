defmodule Clementine.Rollout do
  ...
  defmodule Execution do
    @moduledoc false
    # Loop state for `execute/2` — the runner-facing engine. Distinct from
    # `State` (the legacy run/run_stream engine): this one carries the
    # runner-supplied closures and the accumulated typed usage, and its
    # `prefix_len` marks where history + input end and generated messages
    # begin, so `Result.Completed` can separate the two.
    defstruct [
      :model,
      :system,
      :tools,
  ...
  drives; `run/2` and `run_stream/3` remain the standalone pre-durability
  entry points.
